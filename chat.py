"""
chat.py - AI Gateway Chat Client

A feature-rich CLI client for interacting with AI Lab servers (llama.cpp backend).

Features:
- Multiline input: Alt+Enter (or Escape+Enter, or Ctrl+J) for newlines
- Command autocomplete: Tab to complete /commands
- Conversation persistence: Save, list, and resume conversations
...
class SlashCommandCompleter(Completer):
    ...
    COMMANDS = {
        ...
        "/resume": {"_desc": "Resume a saved conversation"},
        "/delete": {"_desc": "Delete a saved conversation"},
    }
...
class ChatClient:
    ...
    def __init__(self, server_url: str, history_file: str):
        ...
        self.current_conversation_title: Optional[str] = None
    ...
    def _auto_save_conversation(self):
        """Auto-save the current conversation if it has a title."""
        if not self.current_conversation_title or not self.conversation_history:
            return
    ...
    def _prompt_for_title(self) -> Optional[str]:
        ...
        try:
            title = Prompt.ask(
                "[yellow]Name this conversation[/yellow]",
                default="",
                console=console,
            ).strip()
    ...
    def _handle_save_command(self, value: Optional[str]):
        ...
        try:
            conv_id = self.conversation_manager.save(
                messages=self.conversation_history,
                title=title,
                conv_id=self.current_conversation_id,
                system_prompt=self.system_prompt,
            ...
            console.print(f"[green]Conversation saved: {title}[/green]")
            console.print(f"[dim]ID: {conv_id} | Messages: {len(self.conversation_history)}[/dim]")
        except Exception as e:
            console.print(f"[red]Failed to save conversation: {e}[/red]")

    def _handle_new_command(self, value: Optional[str]):
        """Handle the /new command."""
        # Save current conversation if it has content
        if self.conversation_history and self.current_conversation_title:
            self._auto_save_conversation()
            console.print(f"[dim]Saved: {self.current_conversation_title}[/dim]")
        elif self.conversation_history:
            # Ask if user wants to save unsaved conversation
            save_first = Prompt.ask(
                "[yellow]Save current conversation first?[/yellow]",
                choices=["y", "n"],
                default="n",
                console=console,
            )
            if save_first.lower() == "y":
        ...
        # Optionally set title for new conversation
        if value:
            self.current_conversation_title = value.strip()
            console.print(f"[green]New conversation started: {self.current_conversation_title}[/green]")
        else:
            console.print("[green]New conversation started.[/green]")

    def _handle_list_conversations(self):
        """Handle the /conversations command."""
        conversations = self.conversation_manager.list_all()

        if not conversations:
            console.print("[yellow]No saved conversations.[/yellow]")
            console.print("[dim]Use /save to save the current conversation.[/dim]")
            return
        ...
        table.add_column("Preview", width=35, no_wrap=True)
        ...
        for idx, conv in enumerate(conversations, 1):
            ...
            # Truncate preview
            preview = conv.get("preview", "")[:35]
            if len(conv.get("preview", "")) > 35:
                preview += "..."
            ...
            if conv["id"] == self.current_conversation_id:
                table.add_row(
                    str(idx),
                    f"[bold green]{conv['title']}[/bold green]",
        ...
        console.print(table)
        console.print("\n[dim]Use /resume <number> or /resume <id> to resume a conversation[/dim]")

    def _handle_resume_command(self, value: str):
        ...
        try:
            number = int(value)
            conversation = self.conversation_manager.get_by_number(number)
            if not conversation:
                console.print(f"[red]No conversation at position {number}[/red]")
                return
        except ValueError:
            # Try as ID
            conversation = self.conversation_manager.load(value)
            if not conversation:
                console.print(f"[red]Conversation not found: {value}[/red]")
                return
        ...
        self.current_conversation_title = conversation.get("title")

        # Restore system prompt if present
        saved_prompt = conversation.get("system_prompt")
        if saved_prompt:
            self.system_prompt = saved_prompt

        # Restore working directory if present
        saved_wd = conversation.get("working_directory")
        if saved_wd:
            wd_path = Path(saved_wd)
            if wd_path.exists():
                self.working_directory = wd_path

        # Display summary
        console.print(f"\n[green bold]Resumed: {self.current_conversation_title}[/green bold]")
        console.print(f"[dim]Messages: {len(self.conversation_history)} | ID: {self.current_conversation_id}[/dim]")
        ...
        if len(self.conversation_history) >= 2:
            ...
            if last_user and last_assistant:
                console.print("\n[bold]Last exchange:[/bold]")
                console.print(f"  [blue]You:[/blue] {last_user}")
                console.print(f"  [green]Assistant:[/green] {last_assistant}")

        console.print()

    def _handle_delete_command(self, value: str):
        ...
        try:
            ...
            if 1 <= number <= len(conversations):
                ...
                conv_title = conversations[number - 1]["title"]
            else:
                console.print(f"[red]No conversation at position {number}[/red]")
                return
        except ValueError:
            # Try as ID
            conv = self.conversation_manager.load(value)
            if conv:
                conv_id = conv["id"]
        ...
        # Confirm deletion
        confirm = Prompt.ask(
            f"[yellow]Delete '{conv_title}'?[/yellow]",
            choices=["y", "n"],
            default="n",
            console=console,
        )
        ...
        if self.conversation_manager.delete(conv_id):
            console.print(f"[green]Deleted: {conv_title}[/green]")
    ...
    def handle_slash_command(self, command: str) -> bool:
        ...
        if cmd in ["exit", "quit"]:
            ...
            self._handle_save_command(value)
            return True

        elif cmd == "new":
            self._handle_new_command(value)
            return True
            ...
            if value is None:
                console.print("[yellow]Usage: /resume <id or number>[/yellow]")
                console.print("[dim]Use /conversations to see saved conversations[/dim]")
            else:
                self._handle_resume_command(value)
            return True

        elif cmd == "delete":
            if value is None:
                console.print("[yellow]Usage: /delete <id or number>[/yellow]")
                console.print("[dim]Use /conversations to see saved conversations[/dim]")
            else:
                self._handle_delete_command(value)
            return True
    ...
    def export_conversation(self, filepath: str):
        ...
        try:
            ...
            content = [
                "=" * 80,
                "AI Gateway Chat - Conversation Export",
...
"""
        console.print(
            Panel(help_text, title="AI Gateway Chat Help", border_style="cyan", box=box.ROUNDED)
        )
...
[bold cyan]Client Status:[/bold cyan]
  ...
  Conversation: {self.current_conversation_title or 'Unsaved'}{' (saved)' if self.current_conversation_id else ''}
...
"""
    ...
    def run(self):
        ...
        console.print(
            Panel(
                "[bold cyan]AI Gateway Chat v1.0[/bold cyan]\n"
